---
title: "The Probabalistic Variation of the Directional Compound Complement (DCC) Construction in Mandarin Chinese"
author: "Xu Zhang"
output: 
  html_document:
    toc: true
...
## defining factors
```{r defining variables, echo = TRUE, eval = TRUE}
library(base)
...
out_data$is_NP_S_tone_1st<-as.factor(out_data$is_NP_S_tone_1st)
...
```{r response counts, echo = TRUE, eval = TRUE}
library(dplyr)
library(ggplot2)
...
```{r NP Length distribution, echo = TRUE, eval = TRUE, fig.width=6, fig.height=4}
...
```{r conditional inference tree, echo = TRUE, eval = TRUE, fig.width=10, fig.height=10}
library(car)         # to calculate VIFs
library(Hmisc)       # to calculate C values
library(party)       # for ctrees and CRF

tree = ctree(Response ~ TimePeriod2 + log_NP_Length + NP_Givenness + Structural_priming + NP_Concreteness + is_NP_S_tone_1st + Genre + Region, data = out_data,control=ctree_control(maxdepth=5)) # maxdepth specifies the height of the tree. Reduce to reduce complexity.
plot(tree)

```
assessing mdel fitting
```{r tree evaluation, echo = TRUE, eval = TRUE}
...
```{r CRF model fitting, echo = TRUE, eval = TRUE}

set.seed(123) # esnures that we can reproduce results
forest = cforest(Response ~ TimePeriod2 + log_NP_Length + NP_Givenness + Structural_priming + NP_Concreteness + is_NP_S_tone_1st + Genre + Formality + Modal + Region, data = out_data)
...
plotting out CRF
```{r CRF plotting, echo = TRUE, eval = TRUE, fig.width=6, fig.height=4}
...
```{r mixed-effect model fitting, echo = TRUE, eval = TRUE}
...
mixed_model <- glmer(Response ~ TimePeriod2 + log_NP_Length + NP_Concreteness + Formality + (1|Genre) + (1|Region), data = out_data, family=binomial)
...
model assessment
```{r model evaluation, echo = TRUE, eval = TRUE}
library(Hmisc) 
library(knitr)
...
```{r model explainability, echo = TRUE, eval = TRUE}
...
```{r model r square, echo = TRUE, eval = TRUE}
r_squared<-r.squaredGLMM(mixed_model)
kable(r_squared)
```

## Plotting all effects
```{r partial effects, echo = TRUE, eval = TRUE, fig.width=10, fig.height=10}
